...
#if KWIQ
    KHTMLPart *part = 0;
    if (QOBJECT_IS_A((&*child->m_part), KHTMLPart))
      part = static_cast<KHTMLPart *>(&*child->m_part);
#else
...
#if KWIQ
      KHTMLPart *part = 0;
      if (QOBJECT_IS_A(readOnlyPart, KHTMLPart))
	  part = static_cast<KHTMLPart *>(readOnlyPart);
#else      
