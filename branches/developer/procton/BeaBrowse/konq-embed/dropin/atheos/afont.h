...
private:
    ...
    RFont* GetFont() const;
    void SetFont( RFont* pcFont );
    
    QFontInternal* d;
};

//Guess this aint threadsafe.
//Private class.
class RFont {
public:
	RFont():mFont(0), mRefs(1) { mFont = new BFont();}
	~RFont() { delete mFont; }
	
	bool operator==( const RFont& cOther ) const
	{
	    return( mFont == cOther.mFont );
	}
