...
KJSO Screen::get(const UString &p) const
{
  ...
  if (p == "height")
    return Number( int(mode.virtual_height) );
  else if (p == "width")
    return Number( int(mode.virtual_width) );
  else if (p == "colorDepth" || p == "pixelDepth")
    return Number( mode.space );
  else if (p == "availLeft")
    return Number( 0 );
  else if (p == "availTop")
    return Number( 0 );
...
//    config->setGroup("Java/JavaScript Settings");
    ...
    if (false/*config->readBoolEntry("DisableWindowOpen")*/) {
      result = Undefined();
    } else {
...
//        if (newPart && newPart->inherits("KHTMLPart")) {
        if ( dynamic_cast<KHTMLPart*>(newPart) != NULL ) {
...
//  delete config;
  ...
  case MoveBy:
    ...
    {
#ifdef __BEOS__
      BWindow * tl = widget->Window();
      BRect cFrame = tl->Frame();
      BPoint dest = cFrame.LeftTop() + BPoint( args[0].toInt32(), args[1].toInt32() );
      // Security check (the spec talks about UniversalBrowserWrite to disable this check...)
      BScreen screen;
      if ( dest.x >= 0 && dest.y >= 0 &&
           dest.x+cFrame.Width() + 1.0f <= screen.Frame().right &&
           dest.y+cFrame.Height() + 1.0f <= screen.Frame().bottom )
          tl->MoveTo( dest );
#else // __BEOS__
      ...
      os::Desktop cDesktop;
      if ( dest.x() >= 0 && dest.y() >= 0 &&
           dest.x()+tl->width() <= cDesktop.GetResolution().x &&
           dest.y()+tl->height() <= cDesktop.GetResolution().y )
          tl->move( dest );
#endif // __BEOS__
    }
    break;
  case MoveTo:
    if(args.size() == 2 && widget)
    {
      ...
      BWindow * tl = widget->Window();
      BRect cFrame = tl->Frame();
      BPoint dest = BPoint( args[0].toInt32(), args[1].toInt32() );
      // Security check (the spec talks about UniversalBrowserWrite to disable this check...)
      BScreen screen;
      if ( dest.x >= 0 && dest.y >= 0 &&
           dest.x+cFrame.Width() <= screen.Frame().right &&
           dest.y+cFrame.Height() <= screen.Frame().bottom )
          tl->MoveTo( dest );	
#else // __BEOS__
      QWidget * tl = widget->topLevelWidget();
  ...
  case ResizeBy:
    if(args.size() == 2 && widget)
    {
      ...
      BWindow * tl = widget->Window();
      BRect cFrame = tl->Frame();
      BPoint dest( cFrame.Width() + args[0].toInt32() - 1.0f, cFrame.Height() + args[1].toInt32() - 1.0f );
      // Security check (the spec talks about UniversalBrowserWrite to disable this check...)
      BScreen screen;
      if ( cFrame.left+dest.x + 1.0f <= screen.Frame().right &&
           cFrame.top+dest.y + 1.0f <= screen.Frame().bottom &&
           dest.x >= 99 && dest.y >= 99 )
          tl->ResizeTo( dest.x, dest.y );
#else // __BEOS__
      QWidget * tl = widget->topLevelWidget();
      QSize dest = tl->size() + QSize( args[0].toInt32(), args[1].toInt32() );
      // Security check: within desktop limits and bigger than 100x100 (per spec)
      os::Desktop cDesktop;
      if ( tl->x()+dest.width() <= cDesktop.GetResolution().x &&
           tl->y()+dest.height() <= cDesktop.GetResolution().y &&
  ...
  case ResizeTo:
    if(args.size() == 2 && widget)
    {
      ...
      BWindow * tl = widget->Window();
      BRect cFrame = tl->Frame();
      BPoint dest( args[0].toInt32() - 1.0f, args[1].toInt32() - 1.0f );
      // Security check (the spec talks about UniversalBrowserWrite to disable this check...)
      BScreen screen;
      if ( cFrame.left+dest.x + 1.0f <= screen.Frame().right &&
           cFrame.top+dest.y + 1.0f <= screen.Frame().bottom &&
  ...
  case Close:
#ifdef __BEOS__
      printf( "Warning: WindowFunc::tryExecute() Close not implemented\n" );
#else      
...
KJSO FrameArray::get(const UString &p) const
{
  ...
  if ( dynamic_cast<KHTMLPart*>(frame) != NULL ) {
