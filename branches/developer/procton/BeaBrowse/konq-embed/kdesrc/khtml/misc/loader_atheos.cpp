...
//Ouch
#warning Difference between BONE and net_server
//#include <socket.h>    //net_server
...
//Use GetBitmap() in TranslationUtils.h instead.
#if 0
...
static vint32		      g_nWorkerThreadCount = 0;
static vint32		      g_nRunningRequestCount = 0;
...
#warning Difference between BONE and net_server
//	virtual ~HTTPLoader() { if ( m_nSocket != -1 ) closesocket( m_nSocket ); }
...
protected:	
	KURL	m_cURL;
	BString	m_cRequestBuffer;
	BString	m_cClientRequestBuffer;
...
//	std::string				m_cURL;
	...
	std::map<BString,BString>	m_cReceivedMetaData;
...
//		nError = lock_semaphore( g_hJobQueue );
		nError = acquire_sem( g_hJobQueue );
		if ( nError < 0 ) {
			if ( errno == EINTR ) {
				continue;
			} else {
...
void init_loader()
{
	...
	bIsInitiated = true;
	g_hJobQueue = create_sem( 0, "http_job_queue" );
/*	
	for ( int i = 0 ; i < 8 ; ++i ) {
...
static void add_http_request( HTTPLoader* pcLoader )
{
	...
	release_sem( g_hJobQueue );

	if ( g_nWorkerThreadCount < MAX_THREADS && g_nWorkerThreadCount < long(g_cPendingRequests.size() + g_nRunningRequestCount) ) {
		thread_id hTread = spawn_thread( worker_thread, "http_worker", 10, NULL );
		if ( hTread >= 0 ) {
			resume_thread( hTread );
			atomic_add( &g_nWorkerThreadCount, 1 );
		}
...
//	int  nSocket;
	...
	if ( m_nSocket != -1 ) {
#warning difference between net_server and BONE.
//		closesocket( m_nSocket );
	...
	m_cRequestBuffer = (m_cPostData.empty()) ? "GET " : "POST ";
	m_cRequestBuffer += m_cURL.encodedPathAndQuery( 0, true ).ascii();
	m_cRequestBuffer += " HTTP/1.0\r\n";

	m_cRequestBuffer += "User-Agent: Mozilla/5.0 (compatible; ABrowse 0.3; AtheOS\r\n";
	
	m_cRequestBuffer << "Host: " << m_cURL.host().ascii() << "\r\n";
	m_cRequestBuffer += "Connection: close\r\n";
	m_cRequestBuffer += m_cClientRequestBuffer;
	if ( m_cPostData.size() > 0 ) {
		m_cRequestBuffer << "Content-Length: " << m_cPostData.size() << "\r\n";
	}
	m_cRequestBuffer += "\r\n";
	...
	while( nBytesSendt < m_cRequestBuffer.Length() ) {
		int nLen = write( m_nSocket, m_cRequestBuffer.String() + nBytesSendt, m_cRequestBuffer.Length() - nBytesSendt );
		if ( nLen < 0 ) {
...
//			BString cKey( m_cMetaDataList[i], 0, nIndex );
//			BString cValue( m_cMetaDataList[i], nIndex + 1, -1 );
			BString cKey( m_cMetaDataList[i].c_str() );
			BString cValue( m_cMetaDataList[i].c_str() );
			cKey.Remove( 0, nIndex );
			cValue.Remove( nIndex+1, m_cMetaDataList[i].size() - nIndex+1  );
			int i;
//			cKey.Strip();
...
//			cValue.Strip();
			...
			for( i=cValue.Length()-1; i>0; i++ )
			  ...
			  // Check for cookies
			if ( cKey.ICompare( "Set-Cookie" ) == 0 || cKey.ICompare( "Set-Cookie2" ) == 0 ) {
				m_cRecvCookieStr += m_cMetaDataList[i].c_str();
				m_cRecvCookieStr += '\n';
			} else if ( cKey.ICompare( "Location" ) == 0 ) {
				m_cLocationStr = cValue.String();
			} else if ( cKey.ICompare( "Content-type" ) == 0 ) {
				m_cMimeType = cValue.String();
			} else {
				MetadataReceived( cKey.String(), cValue.String() );
			}
		}
	}
...
TransfereJobFile::TransfereJobFile( const DOM::DOMString& cBaseURL, const char* pzPath ) : Request( cBaseURL )
{
	...
	m_bDone	= false;
	m_pcFile = NULL;
	m_pcFile = new BFile( pzPath, B_READ_WRITE );
}
...
{
	m_cReceivedMetaData[BString(cKey.c_str()).ToLower()] = cValue.c_str();
}
	...
	if ( key == "referrer" ) {
	  ...
	  // spelling is "referrer"
		...
		if ( ::strncasecmp( key.utf8().data(), value.utf8().data(), key.utf8().size() - 1 ) == 0 ) {
			...
			m_cClientRequestBuffer += key.utf8().data();
			m_cClientRequestBuffer += ": ";
			m_cClientRequestBuffer += value.utf8().data();
			m_cClientRequestBuffer += "\r\n";
		}
...
{
	QString cData = m_cReceivedMetaData[key.lower().ascii()].String();
	return( cData );
}
...
//	try {
#warning Is not loading the broken image bitmap.
#if 0
