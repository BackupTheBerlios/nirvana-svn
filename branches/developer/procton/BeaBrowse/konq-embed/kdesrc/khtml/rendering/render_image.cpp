 ...
 *
 * $Id: render_image.cpp,v 1.1.1.1 2002/04/27 11:01:55 procton Exp $
 */
//#define DEBUG_LAYOUT
...
//			cWidth, cHeight );
		...
		if ( cWidth > 0 && cHeight > 0 ) {
			BBitmap* pcResized = new BBitmap( BRect(0,0,cWidth,cHeight), B_RGB32 );
			uint8* pDst = (uint8*)pcResized->Bits();
			uint8* pSrc = (uint8*)pix.GetBitmap()->Bits();
			BBitmap* pcSrcBM = NULL;
			int nSrcBPR = pix.GetBitmap()->BytesPerRow();
			if ( pSrc == NULL ) {
//				pcSrcBM = new os::Bitmap( pix.width(), pix.height(), B_RGB32, os::Bitmap::SHARE_FRAMEBUFFER | os::Bitmap::ACCEPT_VIEWS );
				pcSrcBM = new BBitmap( BRect(0,0,pix.width(),pix.height()), B_RGB32, true );
				BView* pcView = new BView( pcSrcBM->Bounds(), "", B_FOLLOW_LEFT | B_FOLLOW_TOP, 0 );
				pcSrcBM->AddChild( pcView );
				pcView->SetDrawingMode( B_OP_COPY );
//				pcView->DrawBitmap( pix.GetBitmap(), pcSrcBM->Bounds(), pcSrcBM->Bounds() );
				pcView->DrawBitmap( pix.GetBitmap(), pcSrcBM->Bounds() );
				pcView->Sync();
				pSrc = (uint8*)pcSrcBM->Bits();
				nSrcBPR = pcSrcBM->BytesPerRow();
	    }
	    memset( pDst, 0, cHeight * pcResized->BytesPerRow() );
	    for ( int y = 0 ; y < cHeight ; ++y ) {
				int sy = (y * pix.height()) / cHeight;
				for ( int x = 0 ; x < cWidth ; ++x ) {
				    int sx = (x * pix.width()) / cWidth;
				    *((uint32*)(pDst + y * pcResized->BytesPerRow() + x * 4)) = *((uint32*)(pSrc + sy * nSrcBPR + sx * 4));
				}
	    }
	    delete pcSrcBM;
...
//			resizeCache.resize(s);
			...
			p->drawPixmap( QPoint( _tx + leftBorder + leftPad, _ty + topBorder + topPad), resizeCache/*, scaledrect*/ );
		}
#else		
                QWMatrix matrix;
