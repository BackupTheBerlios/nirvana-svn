...
#ifdef __BEOS__
	kdDebug() << "Warning: KHTMLView::" << __FUNCTION__ <<"() not setting cursor" << endl;;
#else
...
void KHTMLView::init()
{
	...
	if(!d->paintBuffer) {
		d->paintBuffer = new QPixmap(PAINT_BUFFER_HEIGHT, PAINT_BUFFER_HEIGHT);
	}
	if(!d->tp)
		d->tp = new QPainter();
...
{
	kdDebug()<< "KHTMLView::"<< __FUNCTION__ << "() View: " << viewport()->Name() << " Looper: "
		<< (viewport()->Looper()? viewport()->Looper()->Name(): "NULL") << endl;
	if( viewport()->LockLooper() ) {
		kdDebug() << __FUNCTION__ <<" LooperLocked" << endl;
		viewport()->erase();
		...
		resizeContents(visibleWidth(), visibleHeight());	
		viewport()->Sync();
		viewport()->UnlockLooper();
		kdDebug() << __FUNCTION__ <<" LooperUnlocked" << endl;
	}
}
...
void KHTMLView::drawContents( QPainter *p, int ex, int ey, int ew, int eh )
{
...
#ifdef __BEOS__
		kdDebug() << "KHTMLView::" << __FUNCTION__ << ":1()" << endl;
		p->fillRect(ex, ey, ew, eh, QColor( 255, 255, 255 ) );
#else
	...
	while (py < eh) {
		...
		printf("paintBuffer::bitmap: 0x%x\n", d->paintBuffer->GetBitmap() );
		d->tp->translate(-ex, -ey-py);
#ifdef __BEOS__
		kdDebug() << "KHTMLView::" << __FUNCTION__ << ":2()" << endl;
		d->tp->fillRect(ex, ey+py, ew, ph, QColor( 255, 255, 255 ) );
#else
		d->tp->fillRect(ex, ey+py, ew, ph, palette().normal().brush(QColorGroup::Base));
#endif
		kdDebug() << "\tRendering to paintbuffer" << endl;
		m_part->xmlDocImpl()->renderer()->print(d->tp, ex, ey+py, ew, ph, 0, 0);
		d->tp->end();

		kdDebug() << "\tDraw the rendered paintbuffer" << endl;
		p->drawPixmap(ex, ey+py, *d->paintBuffer, 0, 0, ew, ph);
		py += PAINT_BUFFER_HEIGHT;
	}

#ifdef __BEOS__
//	printf( "Warning: KHTMLView::drawContents:2() not sending event\n" );
#else
...
void KHTMLView::doAutoScroll()
{
	...
	BPoint cMousePos;
	uint32 b;
	GetMouse( &cMousePos, &b, false );
	QPoint pos( cMousePos.x, cMousePos.y );
#else
...
{
#ifdef __BEOS__
	printf( "Warning: KHTMLView::print() not implemented\n" );
#else	
	if(!m_part->xmlDocImpl()) return;
...
void KHTMLView::SetTarget( const BMessenger& cMsgTarget )
{
	d->m_cMsgTarget = cMsgTarget;
}

BMessenger& KHTMLView::Target()
{
	return( d->m_cMsgTarget );
}

void KHTMLView::AllAttached()
{
}

void KHTMLView::AllDetached()
{
}

void KHTMLView::WindowActivated( bool /*bIsActive*/ )
{
}

static int convert_mouse_buttons( uint32 nButtons, uint32 nQualifiers )
{
	int nButtonState = 0;
	if ( nButtons & B_PRIMARY_MOUSE_BUTTON ) {
		nButtonState |= Qt::LeftButton;
	}
	if ( nButtons & B_SECONDARY_MOUSE_BUTTON ) {
		nButtonState |= Qt::RightButton;
	}
	if ( nButtons & B_TERTIARY_MOUSE_BUTTON ) {
		nButtonState |= Qt::MidButton;
	}
	if ( nQualifiers & B_SHIFT_KEY ) {
		nButtonState |= Qt::ShiftButton;
	}
	if ( nQualifiers & B_CONTROL_KEY ) {
		nButtonState |= Qt::ControlButton;
	}
	if ( nQualifiers & B_COMMAND_KEY ) {
		nButtonState |= Qt::AltButton;
	}
	return( nButtonState );
}

void KHTMLView::MouseMoved(const BPoint &cNewPos, uint32 /*nCode*/, const BMessage */*pcData*/)
{
/*	
	NoButton	= 0x0000,
	LeftButton	= 0x0001,
	...
	*/
	...
	const BMessage *aMsg = Window()->CurrentMessage();
	int32 nButtons, nModifiers;
	
	aMsg->FindInt32( "buttons",  &nButtons );
	aMsg->FindInt32( "modifiers",  &nModifiers );
	
	int nButtonState = convert_mouse_buttons( nButtons, nModifiers );
	
	BPoint cGlobalPos = ConvertToScreen( cNewPos );
	QMouseEvent cEvent( QEvent::MouseMove, QPoint( cNewPos.x, cNewPos.y ), QPoint( cGlobalPos.x, cGlobalPos.y ), 0, nButtonState );
	viewportMouseMoveEvent( &cEvent );
}

void KHTMLView::MouseDown(const BPoint &cPosition)
{
	d->m_bMouseDownSeen = true;
	MakeFocus( true );
	...
	int nButtonState = convert_mouse_buttons( nButtons, nModifiers );
//	nButtons = convert_mouse_buttons( 1 << (nButtons-1), 0 );
#warning Why is this needed?
	
	BPoint cGlobalPos = ConvertToScreen( cPosition );
	QMouseEvent cEvent( QEvent::MouseButtonPress, QPoint( cPosition.x, cPosition.y ), QPoint( cGlobalPos.x, cGlobalPos.y ), nButtons, nButtonState );
	
	viewportMousePressEvent( &cEvent );
}

void KHTMLView::MouseUp(const BPoint &cPosition)
{
	if ( d->m_bMouseDownSeen == false ) {
		return;
	}
	d->m_bMouseDownSeen = false;
...
//	nButtons = convert_mouse_buttons( 1 << (nButtons-1), 0 );
	...
	BPoint cGlobalPos = ConvertToScreen( cPosition );
	QMouseEvent cEvent( QEvent::MouseButtonRelease, QPoint( cPosition.x, cPosition.y ), QPoint( cGlobalPos.x, cGlobalPos.y ), nButtons, nButtonState );
	
	viewportMouseReleaseEvent( &cEvent );
}

void KHTMLView::KeyDown(const char *pzRawString, int32 nQualifiers)
{
	BRect cBounds = Bounds();
	int offs = (cBounds.Height() < 30) ? int(cBounds.Height()) : 30;
	if ( nQualifiers & B_SHIFT_KEY ) {
		switch( pzRawString[0] )
		{
		case ' ':
			scrollBy( 0, -cBounds.Height() - offs );
			break;
		default:
			QScrollView::KeyDown( pzRawString, nQualifiers );
			break;
		}
	} else {
		switch ( pzRawString[0] )
...
void KHTMLView::KeyUp(const char *pzRawString, int32 nQualifiers)
{
	QScrollView::KeyUp( pzRawString, nQualifiers );
}

void KHTMLView::FrameResized(float new_width, float new_height)
{
	QScrollView::FrameResized(new_width, new_height );
	
	int w = int( ceil(new_width));
	int h = int( ceil(new_height));
