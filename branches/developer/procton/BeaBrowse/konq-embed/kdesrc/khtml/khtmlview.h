...
#endif
    void SetTarget( const BMessenger& cMsgTarget );
    BMessenger& Target();
#endif
    /**
...
//    virtual void	KeyUp( const char* pzString, const char* pzRawString, uint32 nQualifiers );
    virtual void	MouseMoved( const BPoint& cNewPos, uint32 nCode, const BMessage* pcData );
    virtual void	MouseDown( const BPoint& cPosition );
    virtual void	MouseUp( const BPoint& cPosition );
    virtual void	KeyDown( const char* pzRawString, int32 nQualifiers );
    virtual void	KeyUp( const char* pzRawString, int32 nQualifiers );

//    virtual void	FrameSized( const os::Point& cDelta );
    virtual void	FrameResized( float new_width, float new_height ); //?? Is this the one?
