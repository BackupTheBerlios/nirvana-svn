...
struct URLArgs
{
  ...
  virtual ~URLArgs();
#ifdef __BEOS__
  URLArgs( const BMessage& cData );
  void Flatten( BMessage* pcData ) const;
#endif
...
#ifdef __BEOS__
//  void SetMsgTarget( const BMessenger& cMsgTarget );
//  BMessenger& GetMsgTarget();
  void saveState( BMessage* pcBuffer );
  void restoreState( const BMessage& cBuffer );
