...
URLArgs::URLArgs( const BMessage& cData )
{
    d = 0;
    BString cStr;
    
    for ( int i = 0 ; cData.FindString( "doc_state", i, &cStr ) == 0 ; ++i ) {
		docState.append( QString::fromUtf8( cStr.String() ) );
    }
    BPoint cOffset;
    cData.FindPoint( "offset", &cOffset );
    xOffset = int(cOffset.x);
    yOffset = int(cOffset.y);
    cData.FindString( "service_type", &cStr );
    serviceType = QString::fromUtf8( cStr.String() );

    const void* pData;
    ssize_t	nSize;
    if ( cData.FindData( "post_data", B_ANY_TYPE, &pData, &nSize ) == 0 ) {
		postData.resize( nSize );
		memcpy( postData.data(), pData, nSize );
    }
    cData.FindString( "frame_name", &cStr );
    frameName = QString::fromUtf8( cStr.String() );
    cData.FindBool( "trusted_source", &trustedSource );

    if ( cData.FindString( "content_type", &cStr ) == 0 ) {
		d = new URLArgsPrivate;
		d->contentType = QString::fromUtf8( cStr.String() );
		BString cKey;
		for ( int i = 0 ; cData.FindString( "meta_data_key", i, &cKey) == 0 ; ++i ) {
		    BString cDataStr;
		    cData.FindString( "meta_data", i, &cDataStr);
		    d->metaData[QString::fromUtf8( cKey.String() )] = QString::fromUtf8(cDataStr.String() );
		}
    }
}

void URLArgs::Flatten( BMessage* pcData ) const
{
    ...
    pcData->AddPoint( "offset", BPoint( xOffset, yOffset ) );
    pcData->AddString( "service_type", serviceType.utf8().data() );
    if ( postData.size() > 0 ) {
		pcData->AddData( "post_data", B_ANY_TYPE, postData.data(), postData.size() );
    }
...
#ifdef __BEOS__
void BrowserExtension::saveState( BMessage* pcBuffer )
{
    QByteArray cBuffer;
...
void BrowserExtension::restoreState( const BMessage& cBuffer )
{
    const void* pData;
    ssize_t	nLen;
