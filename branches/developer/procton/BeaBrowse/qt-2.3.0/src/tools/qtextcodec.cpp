/****************************************************************************
** $Id: qtextcodec.cpp,v 1.1.1.1 2002/04/27 11:02:28 procton Exp $
**
...
{
    int l = a.size();
    // implicit conversions of QCStrings to QByteArrays used to give an additional 0 at the end.
    // we should however not remove this 0 for utf16 encoded data.
    if( l > 0 && a.data()[l - 1] == '\0' ) {
	if ( mibEnum() != 1000 || a.data()[l-2] == 0)
	    l--;
    }
    l = QMIN( l, len );
...
QCString QSimpleTextCodec::fromUnicode(const QString& uc, int& len ) const
{
    if ( reverseOwner != this ) {
        ...
        for( ;i < m; i++ )
            (*reverseMap)[i] = '?';
	// small hack to map nbsp to space for all simple codecs. Looks way better than a question mark.
	(*reverseMap)[0xa0] = ' ';
        for( i=128; i<256; i++ ) {
